...
# epochs 파라미터 추가: 기본값 200 (원본 동작 유지)
...
if __name__ == '__main__':
    ...
    # --class_name, --epochs 인자 추가
    parser = argparse.ArgumentParser(description='RD4AD Anomaly Detection')
    parser.add_argument('--class_name', type=str, default=None,
                        help='단일 클래스 지정 (예: carpet). 미지정 시 전체 15개 클래스 실행')
    parser.add_argument('--epochs', type=int, default=200,
                        help='학습 epoch 수 (기본값: 200)')
    args = parser.parse_args()

    item_list = ['carpet', 'bottle', 'hazelnut', 'leather', 'cable', 'capsule', 'grid', 'pill',
                 'transistor', 'metal_nut', 'screw','toothbrush', 'zipper', 'tile', 'wood']

    # class_name 지정 시 해당 클래스만, 미지정 시 전체 실행
